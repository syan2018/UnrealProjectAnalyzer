...
UE_PATTERNS = {
    ...
    "UCLASS": re.compile(r"UCLASS\s*\(([^)]*)\)\s*class\s+(?:\w+_API\s+)?(\w+)", re.MULTILINE),
    "USTRUCT": re.compile(r"USTRUCT\s*\(([^)]*)\)\s*struct\s+(?:\w+_API\s+)?(\w+)", re.MULTILINE),
    "UENUM": re.compile(r"UENUM\s*\(([^)]*)\)\s*enum\s+(?:class\s+)?(\w+)", re.MULTILINE),
    "UINTERFACE": re.compile(
        r"UINTERFACE\s*\(([^)]*)\)\s*class\s+(?:\w+_API\s+)?(\w+)", re.MULTILINE
    ),
    "GENERATED_BODY": re.compile(
        r"GENERATED_(?:BODY|UCLASS_BODY|USTRUCT_BODY)\s*\(\s*\)", re.MULTILINE
    ),
}

# Set of UE macro names that should not be treated as methods
UE_MACRO_NAMES: Set[str] = {
    "UPROPERTY",
    "UFUNCTION",
    "UCLASS",
    "USTRUCT",
...
def is_ue_macro_call(code_text: str) -> bool:
    """
    Check if a code snippet is a UE macro call rather than a method.

    This helps avoid treating UPROPERTY(), UFUNCTION(), etc. as method definitions.

    Args:
        code_text: The code text to check

    Returns:
        True if the code appears to be a UE macro call
    """
    if not code_text:
        return False

    # Check for any UE macro at the start of the text
    stripped = code_text.strip()
    for macro in UE_MACRO_NAMES:
        if stripped.startswith(macro):
            return True
        # Also check if it contains only the macro call (e.g., "UPROPERTY(...)")
        if macro in stripped and re.match(rf"^\s*{macro}\s*\(", stripped):
            return True
